...
- `asyncio` 并发执行标准五阶段；五 AI、单轮、单裁判默认约 17 次网页交互。
- 匿名评审、多轮评审/修订、单裁判或多裁判、八维加权决策分、硬门槛、强制排名和明确淘汰结论。
...
4. **比较式裁判评分**：默认 Kimi 先两两比较再评分。首轮在同一次调用中评估独立答案基线和第 1 轮修订，后续每轮继续使用同一八维标尺；系统计算加权决策分、应用正确性/约束/风险硬门槛，并强制产生唯一排名和淘汰项。
...
当前测试覆盖：模型校验、自然语言气泡、流式 JSON 隐藏、全员完成屏障、脱敏逐轮日志、五平台注册表、严格全自动无人工提示、人工备用、14 次单轮批量协议、12 条互评拆分、畸形评分恢复、加权决策分、逐轮效果对比、在线主持接力、裁判/本地主持回退、至少两份有效回答、多轮、并发、单平台失败、取消、稳定调用 ID、瞬时进度不落库、SQLite、历史恢复、导出、图片头像、阶段条、雷达图和无头启动。
...
- 多裁判模式使用平均加权决策分与平均名次聚合，仍不是统计置信区间，也不能证明事实正确；逐轮增益还可能受到裁判量尺漂移影响。
