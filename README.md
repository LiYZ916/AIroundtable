...
- 提供标准共创、红队压力测试、证据审计、执行决策和创新发散五种讨论策略；所选策略贯穿五个阶段并随历史记录保存。
...
- 讨论历史可按主题、问题或状态搜索；“基于本轮继续”会用结构化摘要创建新一轮上下文，不修改旧记录，也不会回灌网页原始输出。
...
9. 需要针对特定目标加强讨论时，在发送前选择“红队压力测试”“证据审计”“执行决策”或“创新发散”。
10. 左侧可搜索历史并重建完整群聊；右侧“基于本轮继续”会预填可编辑的摘要交接，启动后生成新的讨论记录。
11. 点击“打开运行日志”可查看本轮诊断记录。
...
本项目研究了 [wenwen-0617/roundtable](https://github.com/wenwen-0617/roundtable) 的摘要交接、运行可见性和历史恢复思路，以及 [axtonliu/ai-roundtable](https://github.com/axtonliu/ai-roundtable) 的互评、交叉审计和讨论模式。新增功能均按现有 Python/PySide6 架构独立实现；第一个仓库根目录未发现许可证，因此没有复制其源码。版本、许可证判断、采用与未采用内容详见 [OPEN_SOURCE_REVIEW.md](OPEN_SOURCE_REVIEW.md)。
...
当前测试覆盖：模型校验、五阶段策略注入、自然语言气泡、流式 JSON 隐藏、全员完成屏障、脱敏逐轮日志、五平台注册表、严格全自动无人工提示、人工备用、四 AI 17 次无利益冲突协议、12 条互评拆分、自评分注入拦截、畸形评分恢复、加权决策分、逐轮效果对比、在线主持接力、裁判/本地主持回退、至少两份有效回答、多轮、并发、单平台失败、取消、稳定调用 ID、瞬时进度不落库、SQLite、历史搜索与恢复、结构化摘要交接、导出、复选框真实鼠标点击、设置自动保存与重启恢复、图片头像、阶段条、雷达图和无头启动。
...
| DeepSeek 官方网页 | 实验性 | 独立 Edge | 全自动，失败跳过 | 新增适配器待真实账号端到端验证 |
